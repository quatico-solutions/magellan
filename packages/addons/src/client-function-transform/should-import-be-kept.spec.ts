...
describe("shouldImportBeKept", () => {
    ...
    const createImportDeclaration = (importName: string, modulePath: string, typeOnly = false): ts.ImportDeclaration => {
    ...
    const createMultipleNamedImportsDeclaration = (importNames: string[], modulePath: string): ts.ImportDeclaration => {
    ...
    describe("non-import declarations", () => {
        it("should return false for non-ImportDeclaration nodes", () => {
            ...
            const actual = shouldImportBeKept(variableStatement, sourceFile);
        ...
        it("should return false for function declarations", () => {
            ...
            const actual = shouldImportBeKept(functionDeclaration);
        ...
        it("should return false for class declarations", () => {
            ...
            const actual = shouldImportBeKept(classDeclaration);
        ...
        it("should return false for imports with computed module specifiers", () => {
            ...
            const importDecl = factory.createImportDeclaration(
                ...
                factory.createImportClause(
                    false,
                    undefined,
                    factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("test"))])
                ),
                factory.createIdentifier("computedModulePath") // Not a string literal
            );
            const sourceFile = createSourceFile("const someCode = 'test';");

            const actual = shouldImportBeKept(importDecl, sourceFile);
    ...
    describe("imports that should be kept", () => {
        it("should return true for imports with names not handled by client transformer", () => {
            ...
            const actual = shouldImportBeKept(importDecl);
    ...
    describe("imports that should NOT be kept (handled by client transformer)", () => {
        it("should return false for unused remoteInvoke imports", () => {
            const code = `
                import { ${REMOTE_INVOKE_PARAM_NAME} } from "@quatico/magellan-client";
        ...
        it("should return false for unused Context type imports", () => {
            const code = `
                import { type ${CONTEXT_TYPE_NAME} } from "@quatico/magellan-shared";
        ...
        it("should return false for unused Serialization type imports", () => {
            const code = `
                import { type ${SERIALIZATION_TYPE_NAME} } from "@quatico/magellan-shared";
    ...
    describe("mixed imports (some kept, some not)", () => {
        it("should return true when import contains both handled and unhandled names", () => {
            const importDecl = createMultipleNamedImportsDeclaration([REMOTE_INVOKE_PARAM_NAME, "customFunction"], "@quatico/magellan-client");
        ...
        it("should return true when import contains Context and custom types", () => {
            const importDecl = createMultipleNamedImportsDeclaration([CONTEXT_TYPE_NAME, "CustomType"], "@quatico/magellan-shared");
        ...
        it("should return true when import contains Serialization and other utilities", () => {
            const importDecl = createMultipleNamedImportsDeclaration(
                [SERIALIZATION_TYPE_NAME, "helperFunction", "AnotherType"],
        ...
        it("should return false when import contains only handled names", () => {
            const code = `
                import { ${REMOTE_INVOKE_PARAM_NAME}, ${CONTEXT_TYPE_NAME}, ${SERIALIZATION_TYPE_NAME} } from "@quatico/magellan-shared";
    ...
    describe("different import types", () => {
        ...
        it("should handle default imports without named bindings when no source file", () => {
            const importDecl = factory.createImportDeclaration(
                undefined,
                factory.createImportClause(false, factory.createIdentifier("defaultImport"), undefined),
                factory.createStringLiteral("some-module")
            );

            const actual = shouldImportBeKept(importDecl); // No source file
    ...
    describe("edge cases", () => {
        ...
        it("should handle imports from different module paths correctly", () => {
            ...
            const code = `
                import { ${REMOTE_INVOKE_PARAM_NAME} } from "./custom-remote-invoke";
    ...
    describe("real-world scenarios", () => {
        it("should keep imports for third-party libraries", () => {
            ...
            scenarios.forEach(importDecl => {
                expect(shouldImportBeKept(importDecl)).toBe(true);
            });
        });
        ...
        it("should not keep unused imports that will be added by client transformer", () => {
            ...
            testCases.forEach(testCase => {
                ...
                expect(shouldImportBeKept(importDecl, sourceFile)).toBe(false);
            });
        });
    });
    ...
    describe("external imports from node_modules", () => {
        it("should keep used Node.js built-in imports", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const pathImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const fsImport = sourceFile.statements[1] as ts.ImportDeclaration;

            const pathResult = shouldImportBeKept(pathImport, sourceFile);
            const fsResult = shouldImportBeKept(fsImport, sourceFile);
        ...
        it("should remove unused Node.js built-in imports", () => {
            ...
            const pathImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const fsImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const osImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const pathResult = shouldImportBeKept(pathImport, sourceFile);
            const fsResult = shouldImportBeKept(fsImport, sourceFile);
            const osResult = shouldImportBeKept(osImport, sourceFile);
        ...
        it("should keep used npm package imports", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const lodashImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const axiosImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const uuidImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const lodashResult = shouldImportBeKept(lodashImport, sourceFile);
            const axiosResult = shouldImportBeKept(axiosImport, sourceFile);
            const uuidResult = shouldImportBeKept(uuidImport, sourceFile);
        ...
        it("should keep used type-only imports from @types packages", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const httpImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const expressImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const nodeImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const httpResult = shouldImportBeKept(httpImport, sourceFile);
            const expressResult = shouldImportBeKept(expressImport, sourceFile);
            const nodeResult = shouldImportBeKept(nodeImport, sourceFile);
        ...
        it("should handle mixed named imports from external packages", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const fsImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const pathImport = sourceFile.statements[1] as ts.ImportDeclaration;
        ...
        it("should remove completely unused external package imports", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const momentImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const chalkImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const lodashImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const momentResult = shouldImportBeKept(momentImport, sourceFile);
            const chalkResult = shouldImportBeKept(chalkImport, sourceFile);
            const lodashResult = shouldImportBeKept(lodashImport, sourceFile);
        ...
        it("should handle scoped package imports correctly", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const angularImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const rxjsImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const nestjsImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const angularResult = shouldImportBeKept(angularImport, sourceFile);
            const rxjsResult = shouldImportBeKept(rxjsImport, sourceFile);
            const nestjsResult = shouldImportBeKept(nestjsImport, sourceFile);
        ...
        it("should handle namespace imports from external packages", () => {
            ...
            const fsImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const pathImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const cryptoImport = sourceFile.statements[2] as ts.ImportDeclaration;

            const fsResult = shouldImportBeKept(fsImport, sourceFile);
            const pathResult = shouldImportBeKept(pathImport, sourceFile);
            const cryptoResult = shouldImportBeKept(cryptoImport, sourceFile);
        ...
        it("should distinguish between external and internal imports", () => {
            ...
            const fsImport = sourceFile.statements[0] as ts.ImportDeclaration;
            const axiosImport = sourceFile.statements[1] as ts.ImportDeclaration;
            const utilImport = sourceFile.statements[2] as ts.ImportDeclaration;
            const typeImport = sourceFile.statements[3] as ts.ImportDeclaration;

            const fsResult = shouldImportBeKept(fsImport, sourceFile);
            const axiosResult = shouldImportBeKept(axiosImport, sourceFile);
            const utilResult = shouldImportBeKept(utilImport, sourceFile);
            const typeResult = shouldImportBeKept(typeImport, sourceFile);
    ...
    describe("actual unused import detection", () => {
        it("should detect truly unused imports", () => {
            ...
            `;
            const sourceFile = createSourceFile(code);
            const importDecl1 = sourceFile.statements[0] as ts.ImportDeclaration;
            const importDecl2 = sourceFile.statements[1] as ts.ImportDeclaration;

            const actual1 = shouldImportBeKept(importDecl1, sourceFile);
            const actual2 = shouldImportBeKept(importDecl2, sourceFile);
        ...
        it("should handle imports without source file (fallback behavior)", () => {
            ...
            const actual = shouldImportBeKept(importDecl); // No source file provided
    ...
    describe("redundant import removal", () => {
        ...
        it("should remove unused imports even when mixed with used imports", () => {
            const code = `
                import { ${REMOTE_INVOKE_PARAM_NAME}, ${CONTEXT_TYPE_NAME} } from "@quatico/magellan-shared";
        ...
        it("should remove imports with unused names from any module path", () => {
            ...
            const testCases = [
                { name: REMOTE_INVOKE_PARAM_NAME, path: "./local-remote-invoke" },
                { name: CONTEXT_TYPE_NAME, path: "../types/context" },
                { name: SERIALIZATION_TYPE_NAME, path: "../../shared/serialization" },
                { name: REMOTE_INVOKE_PARAM_NAME, path: "custom-magellan-client" },
            ];
        ...
        it("should preserve imports when only some are unused", () => {
            ...
            const mixedImport = createMultipleNamedImportsDeclaration(
                [REMOTE_INVOKE_PARAM_NAME, "customUtility", CONTEXT_TYPE_NAME, "AnotherType"],
                "@quatico/magellan-shared"
            );

            const actual = shouldImportBeKept(mixedImport);
        ...
        it("should remove unused imports regardless of import type (regular vs type-only)", () => {
            const testCases = [
                { name: REMOTE_INVOKE_PARAM_NAME, module: "@quatico/magellan-client", typeOnly: false },
                { name: CONTEXT_TYPE_NAME, module: "@quatico/magellan-shared", typeOnly: false },
                { name: SERIALIZATION_TYPE_NAME, module: "@quatico/magellan-shared", typeOnly: false },
                { name: REMOTE_INVOKE_PARAM_NAME, module: "@quatico/magellan-client", typeOnly: true },
                { name: CONTEXT_TYPE_NAME, module: "@quatico/magellan-shared", typeOnly: true },
                { name: SERIALIZATION_TYPE_NAME, module: "@quatico/magellan-shared", typeOnly: true },
            ];
        ...
        it("should handle unused imports from scoped packages", () => {
            const testCases = [
                { name: REMOTE_INVOKE_PARAM_NAME, module: "@other-org/magellan-client" },
                { name: CONTEXT_TYPE_NAME, module: "@custom/magellan-shared" },
                { name: SERIALIZATION_TYPE_NAME, module: "@my-company/shared" },
            ];
    ...
    describe("function behavior consistency", () => {
        it("should return consistent results for the same input", () => {
            ...
            const result1 = shouldImportBeKept(importDecl);
            const result2 = shouldImportBeKept(importDecl);
        ...
        it("should handle the same import declaration multiple times", () => {
            ...
            const results = Array.from({ length: 5 }, () => shouldImportBeKept(importDecl, sourceFile));
