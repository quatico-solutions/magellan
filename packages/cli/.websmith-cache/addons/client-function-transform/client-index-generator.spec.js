...
describe("createClientIndexGenerator", () => {
    const createMockContext = (files, outDir = "/project/lib") => {
        const writtenFiles = new Map();
        const mockSystem = {
            fileExists: (path) => path in files || writtenFiles.has(path),
            readFile: (path) => files[path] || writtenFiles.get(path),
            writeFile: (path, content) => {
                writtenFiles.set(path, content);
            },
        };
        const context = {
            getSystem: () => mockSystem,
    ...
    it("should generate index.js with exported async functions", () => {
        ...
        const generator = (0, client_index_generator_1.createClientIndexGenerator)();
    ...
    it("should not generate anything when no outDir configured", () => {
        ...
        const writtenFiles = new Map();
        const mockSystem = {
            fileExists: (path) => path === "/project/lib/service.js",
            readFile: (path) => (path === "/project/lib/service.js" ? jsContent : undefined),
            writeFile: (path, content) => {
                writtenFiles.set(path, content);
            },
        };
