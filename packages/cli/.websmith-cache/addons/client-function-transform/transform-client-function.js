...
const transformClientFunction = (factory, node, declaration, func, namespace, context) => {
    ...
    if (typescript_1.default.isIdentifier(inputParam.name) && inputParam.name.text !== "_") {
        dataProperties.push(factory.createPropertyAssignment(factory.createIdentifier(inputParam.name.text), factory.createIdentifier(inputParam.name.text)));
    }
    else if (!typescript_1.default.isIdentifier(inputParam.name) || inputParam.name.text !== "_") {
